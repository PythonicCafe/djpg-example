...
class EmpresaQuerySet(models.QuerySet):
    def search(self, term, language="portuguese", normalization=8 + 32, min_rank=None):
        ...
        """
        if not term:  # `qs.search(None, None)` = `qs` (shortcut)
            return self

        # The search vector must be the same as in the index
        search_vector = SearchVector("razao_social", "nome_fantasia", config="portuguese")
        query = SearchQuery(term, config=f"pg_catalog.{language}", search_type="websearch")
        qs = self.annotate(
            search_vector=search_vector,
            search_rank=SearchRank(search_vector, query, normalization=normalization),
...
class Empresa5(models.Model):
    ...
    data_opcao_simples = models.DateField(null=True, blank=True)
    data_exclusao_simples = models.DateField(null=True, blank=True)
    data_opcao_mei = models.DateField(null=True, blank=True)
    data_exclusao_mei = models.DateField(null=True, blank=True)
    ...
    codigo_porte = models.SmallIntegerField(blank=True, null=True, choices=choices.PORTE)
    ...
    razao_social = models.TextField(blank=True, null=True, max_length=200)
